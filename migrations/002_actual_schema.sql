-- Migration to match the actual schema used by the application
-- This creates the tables that the codebase actually uses

-- Create users table (simplified version used by the app)
...
-- This allows ON CONFLICT to work with the expression
CREATE UNIQUE INDEX IF NOT EXISTS idx_metrics_account_date_unique
ON metrics("accountId", (DATE("recordedAt")));
